...
Vvveb.Components = {
    ...
    render: function (type) {
        ...
        for (var i in component.properties) {
            ...
            if (property.exclude_ids) {
                var elid = element.attr("id");
                if (property.exclude_ids.indexOf(elid) >= 0)
                    continue;
            }
...
Vvveb.Models = {
    ...
    add: function (type, data) {
        data.type = type;
        this._models[type] = data;
    },
    getComponent: function(type){
        var m = this.get(type);
        var ctype = m.componentType;
        var comp = Vvveb.Components.get(ctype);
        return comp;
    }
};
...
Vvveb.Builder = {
                ...
                for (i in components) {
                    ...
                    if (component) {
                        ...
                        if (component.image) {
                            ...
                            item.css({
                                backgroundImage: "url(" + base_url + 'assets/pagebuilder/libs/builder/' + component.image + ")",
    ...
    _frameLoaded: function () {
        ...
        self.baseContent = $(window.FrameDocument).find("#__core_content_builder").html();
    ...
    selectNode: function (node) {
        ...
        var target = jQuery(node).closest("[brpb-element]");
        if (target.length == 0) {
            return;
        }

        if (target.hasClass("pb-disabled")) {
        ...
        if (target) {
            ...
            // if(target.hasClass("pb-disabled-add")){
            //     $("#add-section-btn").hide();
            // }
            if (target.attr("id") == "__core_content_builder") {
    ...
    /* Init Short code components */
    _initScodeComponents: function(){
        var doc = window.FrameDocument;
        var content = doc.getElementById("__core_content_builder");
        html = $(content);

        var scodes = $(html).find(".brpb-scode-el");
        if(scodes.length >0){
            for (let index = 0; index < scodes.length; index++) {
                var scodel = scodes[index];
                var code = $(scodel).attr("brpb-scode");
                var model = Vvveb.Models.get(code);
                var comp = Vvveb.Models.getComponent(code);
                if(comp.afterInsert){
                    comp.afterInsert( $(scodel) );
                }                
            }
        }
    },
    ...
    _initHighlight: function () {
        ...
        self.frameHtml.on("mousemove touchmove", function (event) {
            ...
            if (event.target && isElement(event.target) && event.originalEvent) {
                target = jQuery(event.target).closest("[brpb-element]");
                if (target.length == 0)
                    return false;
                if (target[0]['id'] !== '') {
                    $('#change_parent_div').val(target[0]['id']);
                } else {
                    $('#change_parent_div').val('');
                }
                ...
                if (self.isDragging) {
                    ...
                    if (!self.designerMode && self.iconDrag)
                        self.iconDrag.css({'left': x + 275/*left panel width*/, 'top': y - 30});
                }// else //uncomment else to disable parent highlighting when dragging
                {
                    ...
                    jQuery("#highlight-name").html(target.attr("brpb-element"));
        ...
        self.frameHtml.on("dblclick", function (event) {
            ...
            if (Vvveb.Builder.isPreview == false) {
                ...
                var target = jQuery(event.target).closest("[brpb-element]");
                if (target.length == 0)
                    return;
                self.texteditEl = target;

                if (target.hasClass("pb-disabled")) {
                    self.texteditEl.attr({'contenteditable': false, 'spellcheckker': false});
                    return;
                } else {
    ...
    _initBox: function () {
        ...
        $("#delete-btn").on("click", function (event) {
            ...
            if (self.selectedEl.attr("id") == "__core_content_builder") {
                self.selectedEl.html('');
            } else {
        ...
        $("#add-section-btn").on("click", function (event) {
            ...
            var parentDiv = $('#change_parent_div').val();

            if (parentDiv === '__core_content_builder') {
                $('#add-section-insert-mode-inside').prop('checked', true);
                $("#add-section-insert-mode-after").attr('disabled', true);
            } else {
                $("#add-section-insert-mode-after").attr('disabled', false);
                // $('#add-section-insert-mode-inside').prop('checked', false);
            ...
            if (((top + addSectionBox.outerHeight()) + self.frameDoc.scrollTop()) > outerHeight)
        ...
        function addSectionComponent(html, component, after = true) {
            var node = $(html);

            if (addSectionElement.closest("#__core_content_builder").length == 0) {
                addSectionElement = Vvveb.Builder.frameBody.find("#__core_content_builder");
                after = false;
            }

            if(component.beforeInsert){
                component.beforeInsert(node);
            }
            ...
            if(component.afterInsert){
                component.afterInsert(node);
            }
        ...
        $(".components-list li ol li", addSectionBox).on("click", function (event) {
            ...
            addSectionComponent(html, component, (jQuery("[name='add-section-insert-mode']:checked").val() == "after"));
    ...
    getContent: function(filterShortcode = true){
        var doc = window.FrameDocument;
        var content = doc.getElementById("__core_content_builder");
        html = $(content);
        if(filterShortcode){
            ...
            if(scodes.length >0){
                for (let index = 0; index < scodes.length; index++) {
                    const scodel = scodes[index];
                    if( $(scodel).find(".brpb-scode-server")){
                        var scode = $(scodel).find(".brpb-scode-server").html();
                        $(scodel).html(scode);
                    }else{
                        $(scodel).html('');
                    }
                    // scodes[index].replaceWith(scodel.outerHTML);
                }
            }
        }
        return html.html();
...
//        html = html.replace("</html>", "");
    ...
    saveAjax: function (fileName, startTemplateUrl, callback) {
        var currentHtml = this.getContent(false);
        var data = {};
        data["pageID"] = Vvveb.FileManager.getCurrentPageId();
        data["html"] = this.getHtml();
        this.setHtml(currentHtml);

        $.ajax({
            type: "POST",
            url: base_url + 'pages/save_page_content/' + data["pageID"], //set your server side save script url
            data: data,
            cache: false,
            success: function (data) {
                var data = jQuery.parseJSON(data);
                if (data.result === 1) {
                    $('#message-modal').modal().find(".modal-body").html("Content saved successfully");
                } else {
...
Vvveb.Helper = {
    generateShCode: function (node, component, items) {
        var code = node.attr("brpb-scode");
        var scode = '[ai:'+code;
        var params_array = [];
        for (let index = 0; index < items.length; index++) {
            const item = items[index];
            var attr_key = 'data-'+item;
            var attr_value = node.attr(attr_key);
            params_array.push(item+'='+attr_value);
        }
